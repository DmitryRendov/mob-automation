...
```sh
$ ansible-playbook -i ./inventory/live mob-server-provision.yml --extra-vars "user=minecraft env=mobserver-vps mode=setup" -v
```
...
```sh
ansible-playbook -i ./inventory/live mob-server-provision.yml --extra-vars "user=minecraft env=mobserver-vps mode=update password=Secret_DB_Pass cooldown=10" -v
```
where:
...
- mode - should be `update` to avoid worlds and players data reset (be carefull).

### MOB site deployment

MOB site is a part of game server and located in this GitHub repo - [mob-site]
```sh
$ ansible-playbook -i ./inventory/live mob-site-deploy.yml --extra-vars "user=minecraft env=mobsite-server" -v
```
