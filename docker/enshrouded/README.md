...
```bash
docker build -t enshrouded .
```

This command will create an image named `enshrouded` that contains all necessary dependencies and configurations.

## Configure the Server

Edit the `enshrouded.json` file if you want to customize the server settings, such as name, `gamePort`, `queryPort`, and other gameplay parameters.
...
```bash
docker run -d --name enshrouded_container --restart always \
...
```bash
docker logs -f enshrouded_container
```
...
- **Stop the Server:**
  ...
  ```bash
  docker stop enshrouded_container
  ```

- **Restart the Server:**

  ```bash
  docker start enshrouded_container
  ```

- **Or just:**

  ```bash
  docker restart enshrouded_container
  ```
...
- Check server logs for error messages using `docker logs -f enshrouded_container`.
